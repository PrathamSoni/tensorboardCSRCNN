...
class SRCNN(object):
  ...
  def build_model(self):
    ...
    #weights
    self.weights = {
      'w1': tf.Variable(tf.random_normal([5, 5, 9, 16], stddev=1e-3), name='w1'),
      'w2': tf.Variable(tf.random_normal([3, 3, 16, 32], stddev=1e-3), name='w2'),
      'w3': tf.Variable(tf.random_normal([1, 1, 32, 64], stddev=1e-3), name='w3'),
      'w4': tf.Variable(tf.random_normal([5,5,64,1], stddev=1e-3), name='w4')
    }
    #bias
    self.biases = {
...
###add momentum###tf.train.GradientDescentOptimizer(config.learning_rate).minimize(self.loss)
  ...
  def model(self):
    ...
    conv4 = tf.nn.conv2d(conv3, self.weights['w4'], strides=[1,1,1,1], padding='SAME') + self.biases['b4']
